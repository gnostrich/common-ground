...
from .language_registry import classify_path
...
def walk_repo(repo_root: str | Path, repo_name: str) -> RepoIntakeReport:
    ...
    for p in paths:
        ...
        if spec.classification == "chart-worthy":
            if not is_chart(spec.chart):
                raise ValueError(
            ...
            doc = Document(
                doc_id=f"repo:{repo_name}:{rel}",
                chart=spec.chart,
                text=text,
                source="repo_adapter",
                meta={"repo": repo_name, "path": rel, "sha256": digest},
            )
