...
class LanguageRegistryIsAPlugInSeam(unittest.TestCase):
    ...
    def test_filename_rules_win_over_extension_rules(self):
        ...
        self.assertEqual(spec.classification, "shelf")  # undeclared extension
        self.assertEqual(spec.rule, "extension:package-lock.json",
                         "filename rules are keyed by basename in the one manifest")

    def test_an_unregistered_extension_defaults_to_shelf_not_a_silent_guess(self):
        # `Makefile` acquired a filename rule in the rebase, so the undeclared case needs an
        # extension nobody has declared. The property under test is unchanged.
        spec = classify_path("thing.qqq")
        self.assertEqual(spec.classification, "shelf")
        self.assertTrue(spec.reason)

    def test_an_artifact_with_NO_extension_is_content_classified_not_shelved(self):
        """A chat message id has no name to key on; shelving it would lose the export."""
        self.assertEqual(classify_path("claude||3f2a-11bb:7").classification,
                         "content-classified")
    ...
    # Rebase note: `.md` moved from reference-tier to content-classified when the main
    # ingest path started reading prose. `.go`/`.js` are still held.
    def test_markdown_and_go_and_js_are_reference_tier_not_shelf_and_not_ingested_now(self):
        # Rebase note: `.md`/`.txt` became content-classified when the main ingest path
        # started reading prose, and `.yaml` became shelf. What is asserted here is the
        # property that survived: none of these reaches a chart by NAME alone.
        for name in ("main.go", "app.js", "script.sh"):
            spec = classify_path(name)
            self.assertIn(spec.classification, ("reference-tier", "content-classified"), name)
            self.assertFalse(spec.chart, name)   # no chart is reached by NAME alone
...
class RepoAdapterWalksTheFixtureRepo(unittest.TestCase):
    ...
    def test_every_walked_file_is_classified(self):
        ...
        for f in self.report.files:
            self.assertIn(f.classification,
                          ("chart-worthy", "content-classified",
                           "reference-tier", "shelf"))
    ...
    def test_reference_tier_files_are_held_not_ingested(self):
        ...
        # README.md is no longer held: it is content-classified and its prose is read.
